...
\ASolution{} $P(B) = P{(B^c)}^c = 0.55$.

\ASolution{} if $P(A) = 0.55, P(B) = 0.55$, and $P(A \cup{} B) = 0.85$, then
$0.30$ of $\omega{}$ is mutually exclusive between $A$ and $B$. Thus, the
remainder of $B$ \emph{is} shared with $B$, which means $P(A \cap{} B) = 0.55 -
0.30 = 0.25$

$P(A \cup{} B) - P(A) = (B \cap{} A^c)$ of $\Omega{}$ that belongs to $A$ overlaps with $B$. Thus, $P(A \cap{} B) = 0.25$
